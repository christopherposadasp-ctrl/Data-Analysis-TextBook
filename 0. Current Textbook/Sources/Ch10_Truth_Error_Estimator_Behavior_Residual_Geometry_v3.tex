...
\CoreCompress{
\section{Multicollinearity and Coefficient Instability}

Chapter~\ref{chap:degrees-of-freedom-subspaces-identifiability} described exact non-identifiability: if one design column is an exact linear combination of the others, then \(\mathbf{X}^T\mathbf{X}\) is singular and the coefficient vector is not uniquely determined.  Multicollinearity is the nearby practical problem.  The columns may not be exactly dependent, so the least-squares estimate is unique, but they can be close enough to dependent that individual coefficients become fragile.

The reason is already visible in the variance formula:
\begin{equation}
  \operatorname{Var}(\widehat{\boldsymbol{\beta}}\mid\mathbf{X})
  =\sigma^2(\mathbf{X}^T\mathbf{X})^{-1}.
\end{equation}
When columns of \(\mathbf{X}\) are nearly linearly dependent, \(\mathbf{X}^T\mathbf{X}\) is close to singular.  Its inverse may then have large entries.  In particular, large diagonal entries of \((\mathbf{X}^T\mathbf{X})^{-1}\) inflate the variances and standard errors of the corresponding coefficient estimates.

The practical consequence is a separation between fitted values and individual coefficient interpretation.  The model may estimate a combined direction of two related predictors well enough for useful fitted values or prediction, while struggling to assign stable separate credit to each predictor.  Thus multicollinearity is mainly an interpretation and inference warning, not automatically a prediction failure.
}{}

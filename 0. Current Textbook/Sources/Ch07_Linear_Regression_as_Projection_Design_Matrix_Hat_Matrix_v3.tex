...
\hline
\textbf{Chapter 7 use} & \textbf{Math Foundations connection} & \textbf{MF textbook location} \\
\hline
...
\hline
Normal-equation bridge & Least squares as minimizing $\|\mathbf{A}\mathbf{x}-\mathbf{b}\|^2$, the normal equations, and gradient-based reasoning & Ch05 Sec. 5.5; Ch12 Secs. 12.4, 12.9, 12.11 \\
\hline
\end{tabular}
\end{center}
}{
...
The Core Reader keeps the geometry ideas from Math Foundations but omits the full Math Foundations textbook-location map.  The main supporting ideas here are vectors in \(\mathbb{R}^n\), dot products, orthogonality, bases, full rank, and least-squares normal equations.
}

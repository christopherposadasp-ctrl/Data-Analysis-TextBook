...
Before developing prediction error and model diagnostics, we need a small set of statistical concepts.  This section reviews expectation, variance, covariance, correlation, conditioning, sampling behavior, and bias.  These ideas will reappear throughout the course.
...
\end{equation}
The probabilities weight the possible outcomes.  Outcomes that are more likely contribute more to the average.  For a continuous random variable with density $p(x)$, the corresponding definition replaces the sum with an integral:
\begin{equation}
  \mathbb{E}[X]=\int x p(x)\,dx.
  \label{eq:expectation-continuous-density}
\end{equation}
...
\begin{equation}
  ...
  \label{eq:covariance-shortcut}
\end{equation}
This follows by expanding the centered product and applying linearity of expectation.  Covariance is symmetric, and covariance with itself is variance:
\begin{equation}
  \operatorname{Cov}(X,Y)=\operatorname{Cov}(Y,X),
  \qquad
  \operatorname{Cov}(X,X)=\operatorname{Var}(X).
  \label{eq:covariance-symmetry-self}
\end{equation}
...
Covariance is sensitive to scaling but not shifting.  For constants $a$, $b$, $c$, and $d$,
\begin{equation}
  \operatorname{Cov}(aX+b,cY+d)=ac\,\operatorname{Cov}(X,Y).
  \label{eq:covariance-affine}
\end{equation}
In particular, \(\operatorname{Cov}(aX+b,Y)=a\operatorname{Cov}(X,Y)\).  The shifts \(b\) and \(d\) disappear because covariance is based on deviations from means; multiplying a variable by a constant multiplies its deviations by that constant.
...
For many random quantities, the same idea gives
\begin{equation}
\begin{aligned}
...
\end{aligned}
  \label{eq:variance-general-sum}
\end{equation}
This is why covariance matters when random quantities are combined or averaged: cross-terms affect the total spread unless they vanish.
...
\subsection{Conditioning and Total Laws}

Conditioning means restricting attention to a known event, group, or predictor value before computing a probability, mean, variance, or covariance.  If \(B\) is an event with positive probability, then
\begin{equation}
  \Pr(A\mid B)=\frac{\Pr(A\cap B)}{\Pr(B)}.
  \label{eq:conditional-probability}
\end{equation}
If \(G\) records which group an observation belongs to, then conditioning on \(G=g\) means computing the relevant quantity within group \(g\).  When the groups form a partition of the sample space,
\begin{equation}
  \Pr(A)=\sum_g \Pr(A\mid G=g)\Pr(G=g).
  \label{eq:total-probability}
\end{equation}
This is the law of total probability: compute the probability within each group, then average those group probabilities using the group frequencies.  The same ingredients also give Bayes' rule:
\begin{equation}
  \Pr(G=g\mid A)
  =
  \frac{\Pr(A\mid G=g)\Pr(G=g)}
       {\sum_h \Pr(A\mid G=h)\Pr(G=h)}.
  \label{eq:bayes-rule-partition}
\end{equation}
Bayes' rule updates the probability of group membership after observing that event \(A\) occurred.

The conditional expectation \(\mathbb{E}[Y\mid G]\) is the mean of \(Y\) after conditioning on the group.  It is itself a random variable because its value depends on the group.  The law of iterated expectation says
\begin{equation}
  \mathbb{E}[Y]=\mathbb{E}\!\left[\mathbb{E}[Y\mid G]\right].
  \label{eq:law-iterated-expectation}
\end{equation}
In words: average within groups, then average the group means.

The law of total variance decomposes total variation into within-group and between-group pieces:
\begin{equation}
  \operatorname{Var}(Y)
  =
  \mathbb{E}\!\left[\operatorname{Var}(Y\mid G)\right]
  +
  \operatorname{Var}\!\left(\mathbb{E}[Y\mid G]\right).
  \label{eq:law-total-variance}
\end{equation}
The first term is the average conditional variance: how much \(Y\) varies within groups.  The second term is the variance of the conditional means: how much the group means differ from one another.

The same idea extends to covariance:
\begin{equation}
  \operatorname{Cov}(X,Y)
  =
  \mathbb{E}\!\left[\operatorname{Cov}(X,Y\mid G)\right]
  +
  \operatorname{Cov}\!\left(\mathbb{E}[X\mid G],\mathbb{E}[Y\mid G]\right).
  \label{eq:law-total-covariance}
\end{equation}
The first term is the average within-group covariance.  The second term is the covariance of the group means.  This is why a marginal relationship can differ from a within-group relationship: the overall association may be driven partly by differences among groups.  Regression uses this same conditioning logic when it asks how the response behaves after accounting for predictors.

\subsection{Statistics, Estimators, and Sampling Behavior}

A \emph{statistic} is any function computed from the observed data.  The sample mean, sample variance, sample correlation, maximum value, and fitted regression coefficients are all statistics.  An \emph{estimator} is a statistic used to estimate an unknown population quantity.  The distinction is about purpose: a statistic is a data summary, while an estimator is a data summary aimed at an unknown target.

Because an estimator depends on the sample, it is random before the data are observed.  If we repeatedly collected new samples from the same population and recomputed the estimator, we would get a distribution of possible estimator values.  That distribution is the estimator's \emph{sampling distribution}.  The \emph{standard error} of an estimator is the standard deviation of its sampling distribution.  Standard errors measure sampling instability: how much the estimate would tend to change from one sample to another.

An estimator \(\widehat{\theta}_n\) is \emph{consistent} for a target parameter \(\theta\) if it converges to \(\theta\) as the sample size \(n\) grows.  The Law of Large Numbers explains why the sample mean is a natural estimator of a population mean.  If \(X_1,\ldots,X_n\) are independent observations with common mean \(\mu\), then
\begin{equation}
...
\end{equation}
The sample mean stabilizes around the population mean as more observations are collected.

The Central Limit Theorem describes the approximate shape of the sample mean's sampling distribution.  Under common regularity conditions, if the observations have mean \(\mu\) and variance \(\sigma^2\), then for large \(n\),
\begin{equation}
  \frac{\bar{X}_n-\mu}{\sigma/\sqrt{n}}
  \approx N(0,1).
  \label{eq:central-limit-theorem}
\end{equation}
Equivalently, \(\bar{X}_n\) is approximately normal with mean \(\mu\) and standard error \(\sigma/\sqrt{n}\).  This is one reason normal approximations, standard errors, confidence intervals, and hypothesis tests appear throughout classical statistical modeling.  The theorem does not say every data set is normal; it says that many averages and estimators built from many small contributions have approximately normal sampling behavior.
...
This course repeatedly uses a small set of statistical ideas: expectation, variance, covariance, correlation, conditioning, sampling behavior, and bias.  The full reference text gives more derivation detail; the Core Reader keeps the definitions and interpretations needed for the chapters that follow.
...
Conditioning means computing probabilities or averages after restricting attention to a known event, group, or predictor value.  If \(G\) records group membership, then the law of total probability says
\[
  \Pr(A)=\sum_g \Pr(A\mid G=g)\Pr(G=g).
\]
Bayes' rule uses the same pieces to update group probabilities after observing an event:
\[
  \Pr(G=g\mid A)
  =
  \frac{\Pr(A\mid G=g)\Pr(G=g)}
...
Three total laws will reappear later:
\begin{align}
  \mathbb{E}[Y]&=\mathbb{E}\!\left[\mathbb{E}[Y\mid G]\right], \\
  \operatorname{Var}(Y)
  &=
  ...
  \operatorname{Cov}\!\left(\mathbb{E}[X\mid G],\mathbb{E}[Y\mid G]\right).
\end{align}
These identities say that overall behavior combines within-group behavior with differences among group means.  Regression uses the same conditioning logic when it studies the response after accounting for predictors.

\subsection{Statistics, Estimators, and Sampling Behavior}

A \emph{statistic} is any function computed from the observed data.  An \emph{estimator} is a statistic used to estimate an unknown population quantity.  Because an estimator depends on the sample, it is random before the data are observed.  Its \emph{sampling distribution} is the distribution we would see by repeatedly collecting new samples and recomputing the estimator.  Its \emph{standard error} is the standard deviation of that sampling distribution.

An estimator \(\widehat{\theta}_n\) is \emph{consistent} for \(\theta\) if it converges to \(\theta\) as \(n\) grows.  The Law of Large Numbers says that, for independent observations with common mean \(\mu\),
\[
  \bar{X}_n=\frac{1}{n}\sum_{i=1}^n X_i\longrightarrow \mu.
\]
The Central Limit Theorem says that many averages have approximately normal sampling behavior:
\[
  \frac{\bar{X}_n-\mu}{\sigma/\sqrt{n}}\approx N(0,1)
\]
for large \(n\), under common regularity conditions.  This is why standard errors, confidence intervals, and hypothesis tests often use normal or nearly normal approximations.

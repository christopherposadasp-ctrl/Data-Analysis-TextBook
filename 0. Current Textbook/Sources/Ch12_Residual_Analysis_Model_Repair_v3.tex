...
\CoreCompress{
Funnel or inverted-funnel patterns suggest nonconstant variance.  A curved smooth trend can suggest missing mean structure, such as a nonlinear predictor effect.  Use the scale-location plot together with residuals-versus-fitted and residuals-versus-predictor plots before choosing a repair.
}{}
...
\CoreCompress{
\section{Diagnosing and Repairing Multicollinearity}

Most diagnostics in this chapter look at residual behavior.  Multicollinearity is different: it is a predictor-side problem.  The fitted model may leave no obvious residual pattern, yet individual coefficients may be unstable because predictors contain overlapping information.

\paragraph{First-pass reconnaissance.}
Scatterplot matrices and predictor-predictor correlations are useful first screens.  Strong pairwise association is an early warning, but low pairwise correlations do not rule out multicollinearity.  One predictor can be explained by a combination of several others even when no single pair looks extreme.

\paragraph{Variance inflation factor.}
A common diagnostic is the variance inflation factor.  For predictor \(X_j\), regress \(X_j\) on the remaining predictors and compute the resulting \(R^2\), written \(R^2_{X_j\mid X_{-j}}\).  Then
\begin{equation}
  \operatorname{VIF}_j
  =\frac{1}{1-R^2_{X_j\mid X_{-j}}}.
\end{equation}
If the other predictors do not explain much of \(X_j\), then \(R^2_{X_j\mid X_{-j}}\) is small and \(\operatorname{VIF}_j\) is near 1.  If the other predictors explain \(X_j\) well, then \(R^2_{X_j\mid X_{-j}}\) is near 1 and the VIF becomes large.  A large VIF means the coefficient attached to \(X_j\) is harder to estimate separately from the other predictors.

Rules of thumb such as \(\operatorname{VIF}>5\) or \(\operatorname{VIF}>10\) are screening rules, not deletion rules.  A large VIF says to look closer.  It does not by itself say that the predictor must be removed.  Also, do not interpret the VIF for the intercept or constant column; the intercept is not a substantive predictor.

\paragraph{Fit, prediction, and interpretation.}
Multicollinearity mainly warns about coefficient interpretation and inference.  Large VIFs often correspond to inflated standard errors, unstable coefficient signs, or coefficients that change sharply when related predictors are added or removed.  A model can still have useful fitted values, a strong overall test, or good validation error.  If prediction is the goal and validation performance is good, keeping collinear predictors may be defensible.  If interpretation or inference is the goal, the same collinearity may be a serious limitation.

\paragraph{Condition-number intuition.}
Rank, condition-number, and small-eigenvalue checks describe the design matrix as a whole rather than one predictor at a time.  They can flag exact or near linear dependence, but their numerical values depend on scaling, so use them as supporting screens rather than standalone rules.

\paragraph{Repair and documentation.}
For exact dependence, remove a redundant column or use a reference category.  For near-collinearity, first ask what the related predictors mean.  Are they duplicates, unit conversions, totals and components, or several measurements of the same underlying construct?  For interpretation, consider dropping one predictor, combining predictors into a meaningful index, or redefining the model.  Ridge regression, principal components, and principal-components regression are advanced remedies, but they are future tools rather than part of this chapter's ordinary least-squares workflow.

Document the decision.  A defensible note should answer: Which predictors were involved? Was the issue exact dependence or near-collinearity? Was the goal prediction or interpretation? What repair, if any, was attempted? Did the conclusion, residual behavior, or validation error change?
}{}
...
When several repairs remain defensible, later chapters develop model-selection criteria, cross-validation, and regularization tools for comparing or stabilizing candidate models; those tools supplement this diagnostic workflow rather than replace it.

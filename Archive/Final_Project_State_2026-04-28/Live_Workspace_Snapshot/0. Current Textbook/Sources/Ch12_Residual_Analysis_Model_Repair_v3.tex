...
\end{equation}
This accounts for the fact that residual variance depends on leverage.  This leverage-adjusted version is often called an internally studentized residual.  Externally studentized residuals use a leave-one-out estimate of \(\sigma\), which we do not need here.  Large standardized or studentized residuals are potential outliers.  A rough screening rule is that values beyond \(\pm 2\) deserve attention and values beyond \(\pm 3\) deserve a closer look.
...
Raw residuals are useful for seeing patterns, but they do not all have the same variance because leverage changes residual variability.  Standardized and internally studentized residuals rescale residuals so unusually large residuals are easier to spot; externally studentized residuals use a leave-one-out estimate of \(\sigma\).  The full reference text gives the detailed formulas.
}
...
An outlier is an observation with a response value far from what the model predicts.  A high-leverage point is an observation with an unusual predictor vector.  A point can be one, both, or neither.

The leverage of observation \(i\) is
\begin{equation}
  h_{ii}=\mathbf{x}_i^T(\mathbf{X}^T\mathbf{X})^{-1}\mathbf{x}_i.
\end{equation}
Here \(\mathbf{x}_i^T\) is the full design row for observation \(i\), including the intercept and any transformed or interaction columns.  High leverage is not automatically bad.  It means the observation is far out in predictor space and can have a strong effect on the fitted model.

A common influence measure is Cook's distance:
\begin{equation}
  D_i
  =\frac{e_i^2}{(p+1)\widehat{\sigma}^2}
  \frac{h_{ii}}{(1-h_{ii})^2}.
\end{equation}
Large Cook's distance suggests that deleting observation \(i\) would noticeably change the fitted model.

\paragraph{Repair logic.}
Do not delete a point merely because it is inconvenient.  First ask:
\begin{enumerate}
  \item Was the observation recorded correctly?
  \item Is it inside the intended population of analysis?
  \item Does it represent an operationally important rare case?
  \item Does the conclusion change if the point is included versus excluded?
\end{enumerate}
A defensible analysis often reports a sensitivity check.
...
\begin{enumerate}
  ...
  \item Were outliers or influential observations investigated?
...
A good residual plot is not proof that the model is true.  A bad residual plot is useful evidence that the model needs work.  Repairs include transformed predictors, transformed responses, interaction terms, different model families, robust methods, and sometimes better data.
